...
def split(filename: str):
    ...
    for i in range(0, len(df), 12):
        ...
        #posOfchamp = {
        #    match_chunk.iloc[i]["champion"]: match_chunk.iloc[i]["position"] for i in range(10)
        #}原本想篩選位子來做對位勝率，考慮到搖擺位因此省略
        ...
        match_data = {
            ...
            "team1": {
                "players": [
                    {
                        "position": match_chunk.iloc[i]["position"],
                        "champion": match_chunk.iloc[i]["champion"],
                    }
                    for i in range(0, 6)
                ],
